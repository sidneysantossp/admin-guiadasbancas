...
class LoginController extends Controller
{
    ...
    public function login($login_url)
    {
        ...
        $loginTypes = [
            ...
            'distributor' => 'distributor_login_url'
        ];
        ...
        $siteDirections = [
            ...
            'distributor' => session()?->get('site_direction') ?? $direction ?? 'ltr'
        ];
        $locals = [
            ...
            'distributor' => session()?->get('local') ?? $lang ?? 'en'
        ];
        $role = null;
    ...
    public function login_attemp($role,$email ,$password, $remember = false){
        $auth= ($role == 'admin_employee' ? 'admin' : ($role == 'distributor' ? 'distributor' : $role));
    ...
    public function submit(Request $request)
    {
        ...
        elseif($request->role == 'distributor'){
            $distributor = Vendor::where('email', $request->email)->where('distributor', 1)->first();
            if($distributor && $distributor->status == 0){
                return redirect()->back()->withInput($request->only('email', 'remember'))
                    ->withErrors([translate('messages.inactive_distributor_warning')]);
            }
        }

        $data=$this->login_attemp($request->role,$request->email ,$request->password, $request->remember);

        if($data == 'admin' ||$data == 'vendor' || $data == 'distributor'){
