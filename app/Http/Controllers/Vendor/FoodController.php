...
class FoodController extends Controller
{
    ...
    public function get_categories(Request $request)
    {
        ...
        return response()->json([
            'options' => '<option value="">---Selecione---</option>' . $categories->map(function ($item) {
                return '<option value="' . $item['id'] . '">' . $item['name'] . '</option>';
            })->implode('')
        ]);
    }
    
    // Métodos do Catálogo de Distribuidores
    public function catalog_distributors()
    {
        // Jornaleiro logado (mantido caso seja necessário em lógicas futuras)
        $currentVendor = Auth::guard('vendor')->user();

        // Contar produtos ativos diretamente na tabela food, ignorando escopos globais de restaurante/zona
        $vendors = \App\Models\Vendor::where('distributor', 1)
            ->where('status', 1)
            ->select('vendors.*')
            ->addSelect(['available_products_count' => \App\Models\Food::withoutGlobalScope(\App\Scopes\RestaurantScope::class)
                ->withoutGlobalScope(\App\Scopes\ZoneScope::class)
                ->selectRaw('COUNT(*)')
        ...
        return view('vendor-views.food.catalog-distributors', compact('vendors'));
    }
    
    public function distributor_catalog($distributor_id)
    {
        $currentVendor = Auth::guard('vendor')->user();
        
        // Buscar distribuidor
        $distributor = \App\Models\Vendor::where('distributor', 1)
            ->where('status', 1)
            ->findOrFail($distributor_id);
            
        // Buscar produtos do distribuidor ignorando escopos globais
        $distributorProducts = Food::withoutGlobalScope(\App\Scopes\RestaurantScope::class)
            ->withoutGlobalScope(\App\Scopes\ZoneScope::class)
            ->where('vendor_id', $distributor->id)
            ->where('status', 1)
            ->with(['category'])
            ->get();
            
        // Buscar produtos já habilitados pelo jornaleiro atual (se existirem)
        $vendorProducts = \App\Models\DistributorVendorProduct::query()
        ...
        // Mapa de vínculos (pivot) por food_id para obter informações como min_quantity, stock_quantity e vendor_price quando existirem
        $vendorPivotsMap = \App\Models\DistributorVendorProduct::query()
            ->where('vendor_id', $currentVendor->id)
            ->where('distributor_id', $distributor->id)
            ->get()
            ->keyBy('food_id');
            
        return view('vendor-views.food.distributor-catalog', compact('distributor', 'distributorProducts', 'vendorProducts', 'vendorPivotsMap'));
    }
    
    public function toggle_distributor_product(Request $request)
    {
        try {
            // Validação dos dados de entrada (sale_price agora é opcional sempre)
            $rules = [
                'distributor_product_id' => 'required|integer|exists:food,id',
                'enable' => 'required|boolean',
                'sale_price' => 'nullable',
            ];
            $validated = $request->validate($rules);
            $enableFlag = (int) $request->input('enable');

            // Normalizar sale_price (aceitar vírgula como separador decimal)
            $rawSalePrice = $request->input('sale_price');
            if (!is_null($rawSalePrice) && $rawSalePrice !== '') {
                $normalizedSalePrice = (float) str_replace(',', '.', $rawSalePrice);
            } else {
                $normalizedSalePrice = null;
            }

            $currentVendor = Auth::guard('vendor')->user();
            // Ignorar escopos globais para permitir buscar o produto de outro vendor (distribuidor)
            $distributorProduct = Food::withoutGlobalScope(\App\Scopes\RestaurantScope::class)
            ->withoutGlobalScope(\App\Scopes\ZoneScope::class)
            ->findOrFail($request->distributor_product_id);

            // Verificar se o produto pertence a um distribuidor
            $distributor = \App\Models\Vendor::where('id', $distributorProduct->vendor_id)
                ->where('distributor', 1)
                ->where('status', 1)
                ->first();

            if (!$distributor) {
                return response()->json(['success' => false, 'message' => 'Produto não pertence a um distribuidor válido']);
            }

            $distributorId = $distributorProduct->vendor_id;

            // Buscar (ou criar) o vínculo na tabela pivot DistributorVendorProduct
            $pivot = \App\Models\DistributorVendorProduct::where('vendor_id', $currentVendor->id)
                ->where('distributor_id', $distributorId)
                ->where('food_id', $distributorProduct->id)
                ->first();

            if ($enableFlag === 1) {
                // Se preço foi informado, validar mínimo
                if (!is_null($normalizedSalePrice)) {
                    $salePrice = (float) $normalizedSalePrice;
                    $minPrice = (float) $distributorProduct->price;

                    if ($salePrice < $minPrice) {
                        return response()->json([
                            'success' => false,
                            'message' => 'O preço de venda deve ser maior ou igual ao preço do distribuidor (R$ ' . number_format($minPrice, 2, ',', '.') . ')'
                        ]);
                    }
                }

                if (!$pivot) {
                    $pivot = new \App\Models\DistributorVendorProduct();
                    $pivot->vendor_id = $currentVendor->id;
                    $pivot->distributor_id = $distributorId;
                    $pivot->food_id = $distributorProduct->id;
                    $pivot->min_quantity = 1;
                    // Usar o estoque do produto original do distribuidor como estoque inicial (fallback 100)
                    $pivot->stock_quantity = ($distributorProduct->item_stock ?? 0) > 0 ? (int) $distributorProduct->item_stock : 100;
                }

                // Habilitar
                $pivot->status = 1;
                // Atualizar preço somente se informado; se ainda não existir preço salvo, usar o do distribuidor
                if (!is_null($normalizedSalePrice)) {
                    $pivot->vendor_price = (float) $normalizedSalePrice;
                } elseif (is_null($pivot->vendor_price)) {
                    $pivot->vendor_price = (float) $distributorProduct->price;
                }
                $pivot->save();

                return response()->json(['success' => true, 'message' => 'Produto habilitado com sucesso!', 'pivot_id' => $pivot->id]);
            } else {
                // Desabilitar produto
                if ($pivot) {
                    $pivot->status = 0;
                    $pivot->save();
                    return response()->json(['success' => true, 'message' => 'Produto desabilitado com sucesso!', 'pivot_id' => $pivot->id]);
                }

                return response()->json(['success' => true, 'message' => 'Produto já estava desabilitado']);
            }
            ...
            // Logar detalhes completos do erro + payload da requisição
            \Log::error('Erro ao processar produto do distribuidor', [
                'error' => $e->getMessage(),
                'file' => $e->getFile(),
                'line' => $e->getLine(),
            ...
            $message = config('app.debug') ? ('Erro interno: ' . $e->getMessage()) : 'Erro interno do servidor. Tente novamente.';
