...
//
    ...
    {
        // Normalizar domínio: se vier sem http/https, prefixar automaticamente com https://
        $domain = trim($request->input('react_domain', ''));
        if ($domain && !preg_match('/^https?:\/\//i', $domain)) {
            $domain = 'https://' . ltrim($domain, '/');
        }
        $request->merge(['react_domain' => $domain]);

        $request->validate([
            'react_domain'=>'required|url'
        ],[
            'react_domain.required'=>translate('messages.doamain_is_required'),
            'react_domain.url'=>'Please enter a valid URL',
        ]);
        
        // Salvar sem validação de licença
        Helpers::businessUpdateOrInsert(['key' => 'react_setup'], [
            'value' => json_encode([
                'status'=>1,
                'react_license_code'=>$request['react_license_code'] ?? 'no-license-required',
    ...
    public function login_url_page(){
        $data=array_column(DataSetting::whereIn('key',['restaurant_employee_login_url','restaurant_login_url','admin_employee_login_url','admin_login_url','distributor_login_url'
        ])->get(['key','value'])->toArray(), 'value', 'key');
    ...
    public function login_url_page_update(Request $request){
        ...
        $request->validate([
            ...
            'distributor_login_url' => 'nullable|regex:/^[a-zA-Z0-9\-\_]+$/u|unique:data_settings,value',
        ]);
        ...
        elseif($request->type == 'distributor') {
            Helpers::dataUpdateOrInsert(['key' => 'distributor_login_url','type' => 'login_distributor'], [
                'value' => $request->distributor_login_url
            ]);
        }
