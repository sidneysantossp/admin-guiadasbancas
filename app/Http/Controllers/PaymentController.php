...
class PaymentController extends Controller
{
    public function __construct(){
        // Garantir que o trait exista antes de tentar estender dinamicamente o controlador
        if (is_dir(app_path('Traits')) && trait_exists(\App\Traits\Payment::class)) {
    ...
    {
        $baseControllerClass = '\\' . get_class($this); // FQCN
        $traitClassName = '\\App\\Traits\\Payment'; // FQCN do trait
        $namespace = __NAMESPACE__;
        $fqcn = $namespace . '\\ExtendedController';

        // Evita redeclaração caso outra parte do sistema já tenha definido a classe
        if (class_exists($fqcn)) {
            return null;
        }

        // Define a classe dentro do mesmo namespace do controlador base
