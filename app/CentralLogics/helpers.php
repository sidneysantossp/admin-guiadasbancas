    ...
    public static function product_data_formatting($data, $multi_data = false, $trans = false, $local = 'en',$maxDiscount=true)
    {
        ...
        if ($multi_data == true) {
            foreach ($data as $item) {
                ...
                $categories = [];
                $category_ids_decoded = json_decode($item?->category_ids);
                if (is_array($category_ids_decoded) || is_object($category_ids_decoded)) {
                    foreach ($category_ids_decoded as $value) {
                ...
                $add_ons_decoded = json_decode($item['add_ons']);
                $add_ons_ids = is_array($add_ons_decoded) ? $add_ons_decoded : [];
                $item['add_ons'] = self::addon_data_formatting(AddOn::whereIn('id', $add_ons_ids)->active()->get(), true, $trans, $local);
                $item['tags'] = $item->tags;
            ...
            $variations = [];
            $categories = [];
            $category_ids_decoded = json_decode($data?->category_ids);
            if (is_array($category_ids_decoded) || is_object($category_ids_decoded)) {
                foreach ($category_ids_decoded as $value) {
                    $categories[] = ['id' => (string)$value->id, 'position' => $value->position];
                }
...
//                return rtrim($awsUrl, '/') . '/' . ltrim($awsBucket . '/' . $path . '/' . $data, '/');
    ...
    public static function react_activation_check($react_domain, $react_license_code){
        // Validação de licença desabilitada: sempre considerar como válido
        return true;
    }
    ...
    public static function react_domain_status_check(){
        ...
        if($data){
            // Forçar status ativo sem checagens externas
            $data['status'] = 1;
            $data['react_platform'] = $data['react_platform'] ?? 'custom';
