...
//                return rtrim($awsUrl, '/') . '/' . ltrim($awsBucket . '/' . $path . '/' . $data, '/');
   ...
   public static function getNextOpeningTime($schedule) {
    ...
    {
        return cache()->remember('forbidden_words_all', 3600, function () {
            try {
                return \App\Models\ForbiddenWord::query()
                    ->where('is_active', true)
                    ->orderBy('language')
                    ->orderBy('word')
                    ->get(['word','language','severity','category','replacement'])
                    ->toArray();
            } catch (\Throwable $e) {
                return [];
            }
    ...
    public static function text_contains_forbidden(string $text, ?string $lang = null): bool
    {
        $textL = mb_strtolower($text);
        $words = self::get_forbidden_words();
        foreach ($words as $w) {
            if ($lang && $w['language'] !== $lang) continue;
            $term = $w['word'];
            if ($term === '') continue;
            $pattern = '/(?<!\\p{L})' . preg_quote($term, '/') . '(?!\\p{L})/iu';
            if (preg_match($pattern, $textL)) {
                return true;
            }
        }
        return false;
    ...
    public static function censor_text(string $text, ?string $lang = null, string $mask = '***'): string
    {
        $result = $text;
        $words = self::get_forbidden_words();
        foreach ($words as $w) {
            ...
            $term = $w['word'];
            if ($term === '') continue;
            $replacement = $w['replacement'] ?? $mask;
            $pattern = '/(?<!\\p{L})' . preg_quote($term, '/') . '(?!\\p{L})/iu';
