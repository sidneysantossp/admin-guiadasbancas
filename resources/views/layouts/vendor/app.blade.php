...
<html dir="{{ $site_direction }}" lang="{{ str_replace('_', '-', app()->getLocale()) }}"  class="{{ $site_direction === 'rtl'?'active':'' }}"><head>
    ...
    <script>
        // Ensure jQuery AJAX requests carry the CSRF token header
        document.addEventListener('DOMContentLoaded', function() {
            try {
                var tokenTag = document.querySelector('meta[name="csrf-token"]');
                if (window.jQuery && tokenTag) {
                    $.ajaxSetup({
...
<script>
  console.debug('[layout] before vendor.min.js', 'jQuery:', typeof window.jQuery, '$:', typeof window.$);
</script>
<script src="{{dynamicAsset('public/assets/admin/js/vendor.min.js')}}"></script>
<script>
  // Ensure jQuery alias early
  if (window.jQuery) {
    window.$ = window.jQuery;
  }
  // Save and disable AMD define to avoid UMD grabbing undefined jQuery
  window.__hs_saved_define = (window.define && window.define.amd) ? window.define : null;
  if (window.__hs_saved_define) {
    window.define = undefined;
  }

  // Load theme.min.js only after jQuery is available, with CDN fallback for jQuery
  (function loadThemeAfterJquery(){
    function hasJQ(){ return typeof window.jQuery === 'function'; }
    function setAlias(){ if (hasJQ()) window.$ = window.jQuery; }
    function injectTheme(){
      setAlias();
      var s = document.createElement('script');
      s.src = "{{dynamicAsset('public/assets/admin/js/theme.min.js')}}";
      s.onload = function(){
        // Restore AMD define if it was present
        if (window.__hs_saved_define) {
          window.define = window.__hs_saved_define;
          window.__hs_saved_define = null;
        }
        console.debug('[layout] theme.min.js loaded, jQuery:', typeof window.jQuery, '$:', typeof window.$);
      };
      s.onerror = function(){
        console.error('[layout] Failed to load theme.min.js');
        if (window.__hs_saved_define) {
          window.define = window.__hs_saved_define;
          window.__hs_saved_define = null;
        }
    ...
    if (hasJQ()) {
      ...
      console.warn('[layout] jQuery not found after vendor.min.js, injecting CDN fallback...');
      var jq = document.createElement('script');
      jq.src = 'https://code.jquery.com/jquery-3.5.1.min.js';
      jq.onload = function(){ setAlias(); injectTheme(); };
      jq.onerror = function(){ console.error('[layout] Failed to load jQuery CDN'); injectTheme(); };
...
<script>
    ...
    function conversationView() {
        ...
        var user_id = getUrlParameter('user');
        var url= '{{url('/')}}/banca/message/view/'+conversation_id+'/' + user_id;
        $.ajax({
            url: url,
            success: function(data) {
    ...
    @if(\App\CentralLogics\Helpers::employee_module_permission_check('order') && $order_notification_type == 'manual')
        ...
        $(document).on('click', '.check-order', function () {
            location.href = '{{url('/')}}/banca/order/list/all';
        });
    ...
    const inputs = document.querySelectorAll('input[type="tel"]');
        ...
        document.addEventListener('keydown', function(event) {
            if (event.ctrlKey && event.key === 'k') {
                ...
                if (opener && opener.offsetParent !== null) { // só aciona se o botão estiver visível
