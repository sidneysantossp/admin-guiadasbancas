...
@section('content')
    <div class="content container-fluid">
        ...
        <form action="{{route('admin.login_url.login_url_page_update')}}" method="post">
            @csrf
            <h5 class="card-title mb-3 pt-3">
                <span class="card-header-icon mr-2"><i class="tio-settings-outlined"></i></span> <span>{{ translate('Página de Login do Distribuidor') }}</span>
            </h5>
            <div class="card">
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-12">
                            <input type="text" hidden  name="type" value="distributor">

                            <div class="__bg-F8F9FC-card">
                                <div class="form-group">
                                    <label  class="form-label">
                                        {{translate('URL de login do distribuidor')}}
                                        <span class="input-label-secondary text--title" data-toggle="tooltip"
                                        data-placement="right"
                                        data-original-title="{{ translate('Adicione URL de login dinâmica e segura para Distribuidores') }}">
                                            <i class="tio-info-outined"></i>
                                        </span>
                                    </label>
                                    <div class="input-group-prepend">
                                        <div class="input-group-text">{{ url('/') }}/login/</div>
                                        <input type="text" placeholder="{{translate('URL de login do distribuidor')}}" class="form-control h--45px" name="distributor_login_url"
                                                required value="{{ $data['distributor_login_url'] ?? null  }}">
                                    </div>
                                </div>
                            </div>
                        </div>
