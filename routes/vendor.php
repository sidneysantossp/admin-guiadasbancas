...
Route::group(['namespace' => 'Vendor', 'as' => 'vendor.'], function () {
    Route::group(['middleware' => ['vendor' ,'maintenance','actch:admin_panel']], function () {
        ...
        Route::post('produto/food-variation-generate', 'FoodController@food_variation_generator')->name('produto.food-variation-generate');
        Route::group(['prefix' => 'produto', 'as' => 'food.', 'middleware' => ['module:food','subscription:food']], function () {
            ...
            Route::get('catalogo-distribuidores', 'FoodController@catalog_distributors')->name('catalog-distributors');
            Route::get('catalogo-distribuidor/{distributor}', 'FoodController@distributor_catalog')->name('distributor-catalog');
            Route::post('toggle-product-status', 'FoodController@toggle_distributor_product')->name('toggle-distributor-product');
        });
        ...
        // Distributor Products (Produtos dos Distribuidores para Jornaleiros)
        Route::group(['prefix' => 'distributor-products', 'as' => 'distributor-products.'], function () {
            Route::get('/', 'DistributorProductsController@index')->name('index');
            Route::get('/show/{id}', 'DistributorProductsController@show')->name('show');
            Route::post('/add-to-cart', 'DistributorProductsController@addToCart')->name('add-to-cart');
            Route::get('/cart', 'DistributorProductsController@cart')->name('cart');
            Route::get('/cart-count', 'DistributorProductsController@cartCount')->name('cart-count');
            Route::post('/update-cart', 'DistributorProductsController@updateCart')->name('update-cart');
            Route::post('/remove-from-cart', 'DistributorProductsController@removeFromCart')->name('remove-from-cart');
            Route::post('/checkout', 'DistributorProductsController@checkout')->name('checkout');
        });

        // Distributor Orders (Pedidos feitos aos Distribuidores)
        Route::group(['prefix' => 'distributor-orders', 'as' => 'distributor-orders.'], function () {
            Route::get('/', 'DistributorProductsController@orders')->name('index');
            Route::get('/show/{id}', 'DistributorProductsController@showOrder')->name('show');
            Route::post('/cancel/{id}', 'DistributorProductsController@cancelOrder')->name('cancel');
        });
    });
